...
> **版本**：v4.4.8（2026-09-03）· 对外文档标准版：只描述使用行为与采用流程，架构与实现细节由 Qomob.AI 内部留存。**本文含第六章定价参考，请勿转发至客户侧**
...
| # | 门槛 | 内容 |
|---|------|------|
| 1 | **质量证据** | 质量溢价**尚待盲测验证**（见 [README 质量证据状态](./README.md)）。**先跑一周三臂盲测试点（由 Qomob.AI 协助执行），结果存档后再决定**。别替工具作保，让数据作保——试点不达标时，正确动作是不采用，而不是调阈值 |
| 2 | **商业授权** | v4.4.4 起转为商业授权模式：对外交付的项目与**商业比稿**须事先获得 Qomob.AI & XSkill.dev 书面授权（见 [README 授权条款](./README.md)） |
| 3 | **数据合规** | 客户数据脱敏后输入；敏感客户本地化部署；签 NDA + DPA。复盘数据的三条回流通道（经验教训 / 效果台账 / 品牌记忆）全部脱敏键控、无预算明细、拒绝真实名称字段（系统强制） |
...
| **Strategy Director / Planner** | 用 Planner + Research Agent 做模型选择 / 案例扫描，人工做"洞察真伪"判断和客户心智建模 | 真正的 consumer insight、客户业务理解、行业人脉情报、策略叙事 | 策略模型匹配、多方案对比评分、竞品数据扫描、策略文档结构化 |
...
| **Producer** | 用 Producer Agent 做排期 / 风险清单；**投放前签效果测量约定，复盘后回写效果台账与品牌记忆** | 供应商关系、现场调度、突发事件应对 | 时间线、风险矩阵、执行校验、复盘草稿生成 |
...
| **Data / MMM Analyst** | 用 MMM Agent 做估算框架和预算优化，人工做业务解释和决策建议 | 业务解释、模型假设的合理性判断、CMO 汇报 | MMM 估算框架、预算优化、敏感性分析（**全程估算示意值，非真实建模**） |
...
├─ 【策略定音】Strategy Director 人工审核冠军策略，重点 challenge "洞察真伪"（人工）
├─ 系统自动完成策略一致性验证（10 min）
└─ Planner 修订后输出 v2 策略

Day 2-3：创意阶段
├─ ECD 召开 Kickoff，分享 v2 策略（人工）
├─ Creative Agent 广撒网 5 个方向（1 hr）→ ECD + CD 人工评选 Top 2（2 hr）
├─ 创意反复不达标时，系统自动暂停请求人工介入
└─ AIGC Agent 生成 Prompt + 素材草稿

Day 4-5：投放 + 执行
├─ Media Agent 跑渠道组合 →【预算定音】Media Director 人工确认分配，叠加媒体主关系（人工）
├─ Producer Agent 输出排期 + 风险矩阵 → Producer 人工核对供应商
└─ 投放前：AE + Producer 与客户签效果测量约定（事先定义成功指标、统计窗口与达标阈值）

Day 6-7：合规 + 提案
├─ 系统自动完成合规四维扫描，发现问题自动暂停
├─ Legal 人工签字确认（人工，必须）
├─ Proposal Agent 整合 8+1 章节 + Business Case + Pitch Script
├─【终稿交付】Proposal Director 人工润色 + 演练（4 hr，人工）
└─ 客户汇报
```
...
T-21 ~ T-14：策略 + 创意
├─ 与 SOP-A Day 1-3 相同，但启用交叉验证模块、创意强制多轮打磨
...
```
T+0：危机触发 → 危机流程自动启动，按紧急程度分级（P0-P3）→ CEO/CMO/ECD 紧急召集
T+0 ~ T+4h：Research 舆情扫描 → Planner 出 3 个应对策略 → Compliance 即时审查 → CEO/Legal 人工签字
T+4h ~ T+24h：Content 出声明/文案（ECD 把关调性）→ Media 紧急投放方案 → 发布 + 监测 + 动态迭代修正
```

### SOP-D：复盘 + 数据回流（v4.4.6 起，越用越懂你）

```
Campaign 结束 T+30/60/90：
├─ 【经验通道】系统生成 Lessons Learned 草稿 → 人工补充判断后归档（下次同行业自动参考）
├─ 【效果通道】按效果测量约定生成结果草稿
│    → 人工两件事：声明数据来源（实测 / 估算）+ 核对脱敏
│    → 确认后入库（效果台账是解锁高级能力的唯一水源）
├─ 【品牌通道】生成品牌记忆增量（新红线 / campaign 摘要）→ 人工确认后合并入档案（脱敏）
└─ 定期查看数据就绪度报告（只读，系统永不自动翻转学习开关）
```

> 价值逻辑：lessons 让下次"长记性"，效果台账让主张有据可依，品牌记忆让同品牌第二次合作免追问——**用得越久，系统越懂你的客户，这是时间沉淀出的差异化。**

---

## 五、一周试点（采用的第一步，v4.4.6）

试点流程与判定协议由 Qomob.AI 在启动时提供。三臂设计：

| 臂 | 是什么 | 回答什么 |
|----|--------|---------|
| A | 4Aos 全流程 | 质量 vs 强基线 |
| B | 资深策略 + 通用 AI 单次生成 | 反"应付式对比"基线 |
| C | 你们自己的传统流程 | 质量 vs 人 + 效率 vs 人 |

判定规则预注册：A 臂质量胜率达标才算赢（具体阈值试点启动时双方确认）；质量不达标时效率数据不构成辩护。**结果存档后，才进入定价谈判与对外宣称。**
...
| "为什么比传统报价贵 20%？" | "因为每个策略经过多角色对抗评审和人工终审，方案的结构完整度与合规把关是可审计的。"（胜率数字待试点数据） |
| "为什么 Performance Bonus 这么高？" | "因为效果测量约定事先定义了成功口径，达成与否用数据说话。" |
...
| **数据安全** | 脱敏后输入；敏感客户本地化部署；NDA + DPA；数据档案脱敏键控（系统强制） |
...
| **数据回流失真** | 已内置防护：效果数据无事先约定的成功口径不入账、估算数据不参与统计（详见效果测量约定） |
...
| 阶段 | 目标 | 行动 |
|------|------|------|
| **Day 1-30：试点** | 用数据回答"值不值" | 跑三臂盲测（2-3 个真实 Brief，流程由 Qomob.AI 协助执行）；结果存档；解决商业授权与数据合规 |
